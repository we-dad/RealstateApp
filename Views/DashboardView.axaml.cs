...
public partial class DashboardView : UserControl
{
    ...
    public DashboardView()
    {
        ...
        try
        {
            ...
            // عداداتك الحالية
            OwnersCountText.Text = new OwnerService(_db).GetAll().Count.ToString();
            UnitsCountText.Text = new UnitService(_db).GetAll().Count.ToString();
            TenantsCountText.Text = new TenantService(_db).GetAll().Count.ToString();
            ContractsCountText.Text = new ContractService(_db).GetAll().Count.ToString();
            ReceiptsCountText.Text = new ReceiptService(_db).GetAll().Count.ToString();
    ...
    private void BuildTopUnitsBarChart(List<UnitYearStatRow> stats)
    {
        var top = stats
    ...
    private static async Task<List<UnitYearStatRow>> GetUnitsYearStatsAsync(SqliteConnection con, int year)
    {
        await using var cmd = con.CreateCommand();
        cmd.CommandText = """
            SELECT 
        ...
        var list = new List<UnitYearStatRow>();
        await using var reader = await cmd.ExecuteReaderAsync();
        while (await reader.ReadAsync())
        {
            list.Add(new UnitYearStatRow
            {
