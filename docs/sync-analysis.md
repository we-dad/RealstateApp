...
## 7. مخاطر معروفة في الرفع (لم تُعالج)
وجدها فحص المراجعة أثناء المزامنة الدورية (مهمة 17)، وتُسجَّل هنا ولا تُعدَّل قبل قرار المطوّر:

- **`MarkSynced` بعد الرفع بلا شرط على القيمة المرفوعة:** الرفع يقرأ الصفوف `IsDirty = 1`، يرفعها، ثم يستدعي `MarkSynced(id)`. لو عدّل المستخدم الصف بين القراءة و`MarkSynced` يُمسح `IsDirty` ولا يُرفع التعديل الجديد أبدًا. المزامنة الدورية كل دقيقتين تزيد الفرصة قليلًا. العلاج المحتمل: شرط في `MarkSynced` على أن الصف لم يتغير منذ القراءة.
- **رفع الأقساط بلا فحص `CanWriteOnline`** (`InstallmentSyncService.PushAllDirtyAsync`) بعكس العقارات: الدور `viewer` أو `tester` لو عنده صفوف `IsDirty = 1` سيحاول الرفع في كل مزامنة، ويفشل بسبب RLS، ويُسجَّل في Console فقط.
- **الصف "السام":** صف يفشل رفعه دائمًا يوقف رفع كل ما بعده في نفس الدورة (الاستثناء يقطع الحلقة)، ويتكرر عند كل مزامنة.
- **الفشل الصامت:** فشل الرفع أو السحب يُكتب في Console فقط، بلا رسالة ولا مؤشر "آخر مزامنة".
